...
const VERIFIED_LAYERS = ['Global Air Traffic', 'Maritime Traffic', 'Seismic Activity', 'Satellite Cloud Cover'];
...
const IntelligenceCenter: React.FC<IntelligenceCenterProps> = ({
  ...
  return (
    <motion.div
      initial={{ x: -400 }}
      animate={{ x: 0 }}
      className={`${mobileOpen ? 'flex' : 'hidden'} fixed inset-x-3 bottom-20 top-20 z-[70] flex-col overflow-hidden rounded-xl glass-panel pointer-events-auto lg:static lg:col-start-1 lg:row-start-2 lg:flex lg:rounded-none`}
    >
      <button
        type="button"
      ...
      <div className={`min-h-0 flex-1 flex-col ${desktopOpen ? 'flex' : 'flex lg:hidden'}`}>
        ...
        <div className="border-t border-accent-dim bg-black/55 p-3 sm:p-4">
          <button
            type="button"
            onClick={onRefresh}
            disabled={isLoading}
